...
{
	global $context, $settings, $options, $txt, $scripturl, $modSettings, $counter, $sourcedir;
	...
	{
		if(!empty($modSettings['keycaptcha_is_active']) && ($modSettings['keycaptcha_is_active'] == 1 && !empty($modSettings['keycaptcha_private_key'])))
		{
			echo '
				<div class="post_verification">
					...
					<tr>
						<td width="100%">
							<b>', $txt['keycaptcha_task_header'], ': </b>', $txt['keycaptcha_task_explain'], '
						</td>
					</tr>
					<tr>
					<td width="100%">';
						require_once($sourcedir . '/keycaptcha_class.php');
						$kc_o = new KeyCAPTCHA_CLASS($modSettings['keycaptcha_private_key']);
						echo '<input type="hidden" id="capcode" name="capcode" value="123" />'.$kc_o->render_js();
						echo '<noscript>', $txt['keycaptcha_message_noscript'], '</noscript>
					</td>
					</tr>
					</table>
				</div>';
