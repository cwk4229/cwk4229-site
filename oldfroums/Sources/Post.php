...
function Post2()
{
	...
	if (!$user_info['is_admin'] && !$user_info['is_mod'] && !empty($modSettings['posts_require_captcha']) && ($user_info['posts'] < $modSettings['posts_require_captcha'] || ($user_info['is_guest'] && $modSettings['posts_require_captcha'] == -1)))
	{
		...
		{
			if(!empty($_POST["capcode"])) //Check the input if this exists, if it doesn't, then the user didn't fill it out.
			{
				...
				{
					loadLanguage('Errors');
					$post_errors = array_merge($post_errors, array(0 => 'wrong_keycaptcha'));
				}
			}
			else
			{
